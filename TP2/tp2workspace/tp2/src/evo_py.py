...
class EvoPy:
    ...
    def generate_tests(self, targetPyFile, maxGenerations, maxTime, output_dir, random_seed):
        ...
        def modify_test(test_suite):
            if len(test_suite) == 0:
                return test_suite
            
            func_tuple = 0
            new_testCall = 0
            # Generate another test to modify the current
            for f in module.children:
                if isinstance(f, pyntch.function.FuncType) and f.name[0] == test_suite[0].function:
                    g = test_generator.TestGenerator(f,random_seed)
                    func_tuple = g.generate_new_test_call()
                    new_testCall = test_executor.TestCall(func_tuple[0], func_tuple[1])
                    break
                     
            print(func_tuple)   
            # Select a test_case to modify
            index = int(random.uniform(0, len(test_suite)))
            test_case = test_suite[index]
            
            arguments = test_case.arguments
            arg_index = int(random.uniform(0, len(arguments)))
            arg = arguments[arg_index]
            
            new = func_tuple[1][arg_index]
            arguments[arg_index] = new
            modified_test_call = test_executor.TestCall(func_tuple[0], arguments)
            test_suite[index] = modified_test_call
        ...
        def fitness(test_suite):
            ...
            for i in range(0, len(test_suite)):
                ...
                res = testExecutor.execute(test_suite[i])
                sys.settrace(None)
